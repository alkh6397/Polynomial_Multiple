...
int main(){
    ...
    polynomial b4;
    polynomial c4;
    char a_str[100] = "3y^7+ 4x^4 -1";
    char b_str[100] = "5x^4- 3y^2+7";
    char a_str2[100] = "4y^7 - 4x^4 -1";
    char b_str2[100] = "2y^7 - 3x +1";
    char a_str3[100] = "15x^4 * y^7  -9y^9  +21y^7  +20x^8";
    char b_str3[100] = "-12x^4 * y^2 + +23x^4 +3y^2  -7";
    char a_str4[100] = "15x^4* y^7* 6z^3  -9y^9 * z^2  +21y^7  +20x^8";
    char b_str4[100] = "-12x^4 * y^2 * z^2  + +23x^4 +3y^2  -7";
    char a_str_norm[100];
    char b_str_norm[100];
    char a_str2_norm[100];
    char b_str2_norm[100];
    char a_str3_norm[100];
    char b_str3_norm[100];
    char a_str4_norm[100];
    char b_str4_norm[100];
    int a_term = 0;
    int b_term = 0;
    int c_term = 0;
    int a_term2 = 0;
    ...
    normalize(a_str2, a_str2_norm);
    normalize(b_str2, b_str2_norm);
    normalize(a_str3, a_str3_norm);
    normalize(b_str3, b_str3_norm);
    normalize(a_str4, a_str4_norm);
    normalize(b_str4, b_str4_norm);

    a_term = parser(&a, a_str_norm);
    b_term = parser(&b, b_str_norm);
    a_term2 = parser(&a2, a_str2_norm);
    b_term2 = parser(&b2, b_str2_norm);
    a_term3 = parser(&a3, a_str3_norm);
    b_term3 = parser(&b3, b_str3_norm);
    a_term4 = parser(&a4, a_str4_norm);
    b_term4 = parser(&b4, b_str4_norm);

    multiple(&a, &b, &c, a_term, b_term, &c_term);
    multiple(&a2, &b2, &c2, a_term2, b_term2, &c_term2);
    multiple(&a3, &b3, &c3, a_term3, b_term3, &c_term3);
    multiple(&a4, &b4, &c4, a_term4, b_term4, &c_term4);
    sort(&c, c_term-1);
    sort(&c2, c_term2-1);
    sort(&c3, c_term3-1);
    sort(&c4, c_term4-1);
    ...
    printf("\n");
    result_print(&c2, c_term2-1);
    printf("\n");
    result_print(&c3, c_term3-1);
    printf("\n");
    result_print(&c4, c_term4-1);
    printf("\n");
    return 0;
}